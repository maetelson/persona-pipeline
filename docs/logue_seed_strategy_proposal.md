...
- `reddit`, `stackoverflow`, `shopify_community`, `hubspot_community`, `google_ads_help_community`, `merchant_center_community`, and `klaviyo_community` are current repo concepts.
- `power_bi_community`, `ga4_help_community`, and `looker_studio_forum` are not current source configs in this repository as of inspection time.
- `merchant_center_help_community` does not exist by that exact name in the repo; the closest current source is `merchant_center_community`.
...
| `google_ads_help_community` | `stats wrong`, `report mismatch`, `attribution issue`, `drop in impressions`, `what changed`, `which metric is right` | `attribution_tracking_issue`, `numbers_mismatch`, `trend_interpretation_confusion`, `source_of_truth_conflict`, `funnel_conversion_drop` | Important, but should sit alongside reporting-trust and interpretation pain |
| `merchant_center_help_community` | `products not showing`, `feed not updating`, `price mismatch`, `free listings issue`, `policy says one thing data says another` | `numbers_mismatch`, `source_of_truth_conflict`, `aggregation_filter_issue`, `dashboard_confusion` | Usually not central; this community is more catalog/data-state interpretation than conversion |
...
| `ga4_help_community` | `GA4 says one thing backend says another`, `event count wrong`, `attribution confusing`, `what changed in traffic`, `date range confusion` | `attribution_tracking_issue`, `numbers_mismatch`, `trend_interpretation_confusion`, `aggregation_filter_issue`, `source_of_truth_conflict` | Material, but only one axis within wider measurement pain |
| `looker_studio_forum` | `report says one thing export says another`, `blended data mismatch`, `filter/date issue`, `which source is right`, `can't explain this chart` | `numbers_mismatch`, `aggregation_filter_issue`, `dashboard_confusion`, `source_of_truth_conflict`, `stakeholder_explanation_pressure` | Usually secondary to report interpretation and trust issues |
