"""Normalizer for official BI community records."""
...
class OfficialCommunityNormalizer(PassThroughNormalizer):
    """Normalize official community threads and replies."""
    ...
    def normalize_row(self, row: dict[str, Any]) -> NormalizedPost:
        ...
        return NormalizedPost(
            source=row["source"],
            source_group=row.get("source_group", "official_communities"),
            ...
            parse_version=row.get("parse_version", "official_community_v1"),
            ...
            dedupe_key=make_dedupe_key(row.get("source", ""), row.get("thread_title", title), body),
