"""Normalizer for review-site raw records."""
...
class ReviewSiteNormalizer(PassThroughNormalizer):
    """Normalize review-site rows into the shared schema."""

    def normalize_row(self, row: dict[str, Any]) -> NormalizedPost:
        title = clean_text(row.get("title"))
        body = clean_text(row.get("body_text") or row.get("body"))
        raw_text = combine_text(title, body)
        return NormalizedPost(
            source=row["source"],
            source_group=row.get("source_group", "review_sites"),
            source_name=row.get("source_name", row.get("source", "")),
            source_type=row.get("source_type", "review"),
            ...
            parse_version=row.get("parse_version", "review_v1"),
            ...
            dedupe_key=make_dedupe_key(row.get("source", ""), row.get("product_or_tool", ""), title, body),
